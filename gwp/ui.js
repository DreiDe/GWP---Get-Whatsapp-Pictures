...
function check() {//generate window.Store object after page load
    if (document.getElementsByClassName('rAUz7')[0] != undefined) {
        window.Store = {};
        webpackJsonp([], {"dgfhfgbdeb": (x, y, z) => window.Store.Wap = z('"dgfhfgbdeb"')}, "dgfhfgbdeb");
...
function check2(){//check if all base64 strings exists
	if(index == limit && count * 3 == arr.length){
		if(arr.length != 0){
		console.log(arr);
		window.postMessage({ type: "FROM_PAGE", text: JSON.stringify(arr)}, "*");//send data to content.js			
		}
else{
